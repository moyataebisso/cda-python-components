...
class MqttClientPerformanceTestWithTLS(unittest.TestCase):
    """
    Lab Module 10 - MQTT Performance Test with TLS
    PIOT-CDA-10-001: Test with TLS enabled
    """
    ...
    def setUp(self):
        self.mqttClient = MqttClientConnector(
            clientID = 'CDAMqttClientTLSPerformanceTest001', 
            enableTls = True,
    ...
    def testConnectAndDisconnectTLS(self):
        startTime = time.time_ns()
        ...
        logging.info("Connect and Disconnect (TLS): " + str(elapsedMillis) + " ms")

    def testPublishQoS0TLS(self):
        self._execTestPublish(self.MAX_TEST_RUNS, 0)

    def testPublishQoS1TLS(self):
        self._execTestPublish(self.MAX_TEST_RUNS, 1)

    def testPublishQoS2TLS(self):
        self._execTestPublish(self.MAX_TEST_RUNS, 2)
    ...
    def _execTestPublish(self, maxTestRuns: int, qos: int):
        ...
        logging.info("Publish message TLS - QoS " + str(qos) + " [" + str(maxTestRuns) + "]: " + str(elapsedMillis) + " ms")
